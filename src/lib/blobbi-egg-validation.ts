...
export function getColorRarity(color: string, type: 'base' | 'secondary'): 'common' | 'uncommon' | 'rare' | 'legendary' | null {
  const colorSets = type === 'base' 
    ? { ...VALID_BASE_COLORS, ...ALTERNATIVE_BASE_COLORS }
...
export function getSpecialMarkRarity(mark: string): 'common' | 'uncommon' | 'rare' | 'legendary' | null {
...
export function validateEggProperties(properties: {
  ...
  if (properties.base_color && !isValidBaseColor(properties.base_color)) {
    errors.push(`Invalid base color: ${properties.base_color}. Must be one of the specification-approved colors.`);
  }

  if (properties.secondary_color && !isValidSecondaryColor(properties.secondary_color)) {
    errors.push(`Invalid secondary color: ${properties.secondary_color}. Must be one of the specification-approved colors.`);
  }
  ...
  if (properties.pattern && !isValidPattern(properties.pattern)) {
    errors.push(`Invalid pattern: ${properties.pattern}. Must be one of: ${VALID_PATTERNS.join(', ')}.`);
  }

  if (properties.egg_status && !isValidEggStatus(properties.egg_status)) {
    errors.push(`Invalid egg status: ${properties.egg_status}. Must be one of: ${VALID_EGG_STATUSES.join(', ')}.`);
  }

  if (properties.special_mark && !isValidSpecialMark(properties.special_mark)) {
    errors.push(`Invalid special mark: ${properties.special_mark}. Must be one of the specification-approved marks.`);
  }

  if (properties.title && !isValidTitle(properties.title)) {
    errors.push(`Invalid title: ${properties.title}. Must be one of the specification-approved titles.`);
  }
